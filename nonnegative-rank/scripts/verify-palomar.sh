...
for file in \
  ...
  NonnegativeRankChallenge.lean NonnegativeRankSolution.lean \
  NonnegativeRankExamples.lean comparator.json formalization.yaml \
  ...
  scripts/palomar-core-notation-audit.manifest.json \
  scripts/fake-landrun.sh scripts/landrun-wrapper.sh; do
  test -f "$file" || { echo "error: missing $file" >&2; exit 1; }
done

python3 scripts/check-shape.py "$root"

challenge_dependencies=$(lake env lean --src-deps NonnegativeRankChallenge.lean)
...
lake env lean NonnegativeRankExamples.lean >/dev/null

echo "Standalone nonnegative-rank preparation checks passed."

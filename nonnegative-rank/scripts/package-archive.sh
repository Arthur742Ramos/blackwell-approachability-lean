...
repository_root=$(git -C "$entry_root" rev-parse --show-toplevel)
entry_name=$(basename "$entry_root")
cd "$repository_root"

output=${1:-"$entry_root/.cache/$entry_name.zip"}
force=0
if [ "${2:-}" = "--force" ]; then
  force=1
elif [ "${2:-}" != "" ]; then
...
git archive --format=zip --prefix="$entry_name/" "$commit:$entry_name" -o "$output"
...
while IFS= read -r member; do
  ...
  case "$member" in
    */._*|__MACOSX/*|*/.lake/*|*/.cache/*|*/submission/*|*/output/*|*/heaps/*)
      echo "error: generated or hidden archive member: $member" >&2
      bad_members=1
      ;;
  esac
done < <(unzip -Z1 "$output")
if [ "$bad_members" -ne 0 ]; then exit 1; fi

...
cache_root="$root/.cache/palomar-comparator"
...
if [ "$(uname -s)" != "Linux" ] && [ "${PALOMAR_ALLOW_UNSANDBOXED_LOCAL:-0}" != "1" ]; then
  echo "error: real Landrun requires Linux Landlock" >&2
  echo "set PALOMAR_ALLOW_UNSANDBOXED_LOCAL=1 for an explicit macOS replay" >&2
  exit 1
fi
...
project_toolchain=$(tr -d "[:space:]" < "$root/lean-toolchain")
export_toolchain=$(tr -d "[:space:]" < "$lean4export_dir/lean-toolchain")
if [ "$project_toolchain" != "$export_toolchain" ]; then
  echo "error: project Lean toolchain does not match pinned Lean4Export" >&2
  exit 1
fi
...
if [ "$(uname -s)" != "Linux" ]; then
  landrun_binary="$root/scripts/fake-landrun.sh"
  echo "warning: using explicit unsandboxed macOS fallback; CI uses real Landrun" >&2
fi
cd "$root"
...
COMPARATOR_LANDRUN="$root/scripts/landrun-wrapper.sh" \

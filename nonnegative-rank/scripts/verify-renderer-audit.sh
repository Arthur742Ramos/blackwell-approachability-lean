...
requests = [("theorem", name) for name in config.get("theorem_names", [])]
if not requests:
    raise SystemExit("error: Comparator configuration has no selected theorems")
...
expected = [name for _, name in requests]
if [row.get("name") for row in rows] != expected:
    raise SystemExit("error: renderer audit did not return selected declarations in order")

...
phi-regret instance; that every invertible canonical properizer would induce a
nonzero common invariant; that the explicit skew family has no such invariant;
...
/-- An invertible canonical properizer would transport the simplex's affine
sum functional to a nonzero vector invariant for every comparator. -/

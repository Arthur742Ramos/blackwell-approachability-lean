...
The three selected theorems establish that the family is a valid improper
phi-regret instance, that every canonical properizing matrix is singular, and
therefore that no invertible canonical proper reduction exists.  The result is
scoped to this canonical normal form; it does not claim to formalize the
paper's separate rate/minimality argument that derives that form from its most
general bidirectional affine definition of linear equivalence.
-/

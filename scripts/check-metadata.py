...
assert 'version := v!"0.11.0"' in (root / "lakefile.lean").read_text(encoding="utf-8")
...
assert "one-way affine action/loss" in research_context["selected_result"]
assert data["sources"]
...
assert "Twenty-seven selected statements" in data["status"]["scope"]
